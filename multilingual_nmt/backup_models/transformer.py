...
def sentence_block_embed(embed, x, weights=None):
...
class Transformer(nn.Module):
    def __init__(self, config):
        ...
        # For attack
        # self.weights = nn.Parameter(torch.zeros(config.n_vocab))
        ...
        if self.label_smoothing > 0:
            ...
            #                             size_average=False, reduction='none')
    ...
    def make_input_embedding(self, embed, block, weights=None, index=-2, vocab=None,is_source=None):
        batch, length = block.shape
        emb_block = sentence_block_embed(embed, block, weights)
        if self.weights is not None and is_source:
            if vocab:
                word_embedding = torch.matmul(nn.functional.softmax(self.weights, -1), embed.weight[vocab])
            else:
                word_embedding = torch.matmul(nn.functional.softmax(self.weights, -1), embed.weight)
            emb_block[:, index, :] = word_embedding
    ...
    def forward(self, x_block, y_in_block, y_out_block, get_prediction=False,
                z_blocks=None, index=-2, vocab=None, ex_block=None):
        batch, x_length = x_block.shape
        batch, y_length = y_in_block.shape
        # print('-------------------------------------')
        # print(x_block.shape, y_in_block.shape)
        # print('-------------------------------------')

        if z_blocks is None:
            if ex_block is None:
                ex_block = self.make_input_embedding(self.embed_word,
                                                 x_block, self.weights, index, vocab,is_source=True)
            xx_mask = self.make_attention_mask(x_block,
                                               x_block)

            # print(ex_block.shape, xx_mask.shape)
            xpad_obj = None
