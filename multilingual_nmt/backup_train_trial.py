...
def main():
    ...
    for epoch in range(1):
        ...
        # for num_steps, train_batch in batch_iterator(train_iter):
            ...
            #rand_sent = np.random.choice(vocab,size=2*len(train_batch[0][0])//3)
            #rand_sent = np.random.permutation(train_batch[0][0])#,size=len(train_batch[0][0]))
            ...
            for iter in range(5):
                ...
                for index in torch.randperm(len(train_batch[0][0])):
                    index = index - len(train_batch[0][0]) # make index negative
                    in_arrays = utils.seq2seq_pad_concat_convert(train_batch, -1)
                    count = 0
                    value = -1
                    model.weights = None
                    ...
                    optimizer = torch.optim.Adam(params,
                                              ...
                                              eps=args.optimizer_adam_epsilon)
                    #in_arrays = utils.seq2seq_pad_concat_convert(train_batch, -1)
                    #print(in_arrays[0],index)
                    for i in range(max_iteration):
                        loss, stat = model(*in_arrays, index=index-1, vocab=vocab)
                        #print(loss.item())
                        loss2 = torch.max(0.9-torch.max(nn.functional.softmax(model.weights, -1)),torch.FloatTensor([0]).cuda())
                        #print(torch.max(nn.functional.softmax(model.weights, -1)).item(),loss2.item(),loss.item())
                        #optimizer.zero_grad()
                        #if i>138:
                        #    optimizer.zero_grad()
                        #optimizer.zero_grad()
                        ...
                        probabilities = nn.functional.softmax(model.weights, -1)
                        #probabilities.retain_grad()
                        #print(torch.sum(torch.abs(probabilities.grad)))
                        # p_max, max_index = probabilities.topk(1)
                        p_max = probabilities.max().item()
                        # max_index = probabilities.argmax()
                        max_index = vocab[probabilities.argmax()]
                        # p_max = p_max[0].item()
                        #print(loss.item(),p_max,max_index)
                        ...
                        if p_max > threshold:
                            ...
                            if value == -1:
                                ...
                                if count == max_count and value == max_index:
                                    break
                    ...
                    # Trial code ends
                    # when capping min_loss to orig_loss also check if replacement is actually made e.g. for num_steps=172
                    old_word = train_batch[0][0][index]
                    new_word = vocab[model.weights.argmax()]
                    if min_loss > loss and new_word!=old_word and do_replace:
                        ...
                        new_word = vocab[model.weights.argmax()]
                        src[index] = new_word
                        weight_emb = model.make_input_embedding(model.embed_word,in_arrays[0], weights=None,index=index-1,vocab=vocab,is_source=True)
                        model.weights = None
                        #train_batch[0] = (src,train_batch[0][1])
                        #in_arrays = utils.seq2seq_pad_concat_convert(train_batch, -1)
                        #print(index,new_word,in_arrays[0])
                        proj_emb = model.make_input_embedding(model.embed_word,in_arrays[0], weights=None,index=index,is_source=True)
                        proj_emb1 = proj_emb
                        #print('proj_embed_before:', proj_emb[:, index-1, :])
                        train_batch[0] = (src,train_batch[0][1])
                        in_arrays = utils.seq2seq_pad_concat_convert(train_batch, -1)
                        #print(index,new_word,in_arrays[0])
                        proj_emb = model.make_input_embedding(model.embed_word,in_arrays[0], weights=None,index=index,is_source=True)
                        #print('proj_embed_after:', proj_emb[:, index-1, :])
                        #print(torch.max(torch.abs(weight_emb[0,index-1,:]-proj_emb[0,index-1,:])),torch.mean(weight_emb[0,index-1,:]-proj_emb[0,index-1,:]))
                        print('Cosine similarity:',torch.nn.functional.cosine_similarity(weight_emb[:,index-1,:],proj_emb[:,index-1,:]))
                        #print(weight_emb[:,index,:].shape)

                        #torch.save([weight_emb, proj_emb1, proj_emb], 'embeddings.pt')
                    train_batch[0] = (src,train_batch[0][1])
                    #print('len_src:', len(src), in_arrays[0].shape)
                    print('loss({})\tmin_loss({})\tindex({})\tprob({})'.format(loss.item(), min_loss, index, p_max))
                if flag == False:
                    break
            #new_word = model.weights.argmax()
            #src = train_batch[0][0]
