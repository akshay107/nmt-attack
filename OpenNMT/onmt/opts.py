...
def attack_opts(parser):
    ...
    group.add('--attack_type', '-attack_type', default='softmax',
              choices=['softmax', 'bruteforce', 'fullytargeted', 'ftsoftmax', 'hotflip', 'fthotflip'],
              help='Type of attack to perform. '
                   'Options are [softmax|bruteforce|ftbruteforce|ftsoftmax|hotflip|fthotflip].')

    group.add('--output_adv', '-output_adv', required=True,
              help='Output file directory for adversarial examples')

    group.add('--position', '-position', default=-2, type=int,
              help='Position on which attack should be performed')
